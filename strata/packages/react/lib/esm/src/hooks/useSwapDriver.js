            ...
            try {
                ...
                await swap({
                    ...
                    async postInstructions({ isLast, amount, isBuy }) {
                        ...
                        const instructions = [];
                        const signers = [];
                        let associatedAccountKey = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, entangledBase, owner);
                        if (isBuy) {
                            ...
                            signers.push(...s1);
                            associatedAccountKey = await Token.getAssociatedTokenAddress(ASSOCIATED_TOKEN_PROGRAM_ID, TOKEN_PROGRAM_ID, entangledTarget, owner);
                        }
                        instructions.push(Token.createCloseAccountInstruction(TOKEN_PROGRAM_ID, associatedAccountKey, owner, owner, []));
                        return {
                            instructions,
                            signers,
                            output: null,
