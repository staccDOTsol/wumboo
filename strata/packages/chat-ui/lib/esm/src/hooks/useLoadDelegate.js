...
async function runLoadDelegate(delegateWalletKeypair, chatSdk, sol) {
    if (chatSdk) {
        ...
        if (!structExists) {
            if (!delegateWalletKeypair) {
                ...
                const { instructions: settingsInstructions, signers: settingsSigners } = await chatSdk.initializeSettingsInstructions({
                    settings: {
            ...
            const { instructions: delInstructions, signers: delSigners } = await chatSdk.initializeDelegateWalletInstructions({
                delegateWalletKeypair,
            });
            instructions.push(...delInstructions);
            signers.push(...delSigners);
        }
        const balance = (await chatSdk.provider.connection.getAccountInfo(delegateWalletKeypair.publicKey))?.lamports;
        ...
        await sendInstructions(chatSdk.errors || new Map(), chatSdk.provider, instructions, signers);
    }
}
export function useLoadDelegate() {
    ...
    const { execute: loadDelegate, error, loading, } = useAsyncCallback(runLoadDelegate);
