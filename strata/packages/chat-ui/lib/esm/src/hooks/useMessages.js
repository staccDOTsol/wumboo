...
import { truthy, useEndpoint, usePublicKey, useTransactions, } from "@strata-foundation/react";
...
const txToMessages = {};
async function getMessagesFromTxs(chat, chatSdk, txs = []) {
    if (chat && chatSdk) {
        const failedTx = new Set(Array.from(txs.filter((tx) => tx.meta?.err).map((tx) => tx.signature)));
        const newParts = (await Promise.all(txs.map(async ({ logs, signature: sig, transaction, pending, meta, blockTime, }) => {
            if (!txToMessages[sig] ||
                txToMessages[sig].length == 0 ||
                // @ts-ignore
                (txToMessages[sig][0].pending && !pending)) {
                try {
                    const found = (await chatSdk.getMessagePartsFromInflatedTx({
                        logs,
                        transaction,
                        txid: sig,
                        meta,
...
const lambdaFetcher = async (args) => {
...
export function useMessages({ chat, accelerated, numTransactions = 50, fetcher, }) {
    ...
    const { result: txMessages, loading, error, } = useAsync(getMessagesFromTxs, [stablePubkey, chatSdk, transactions]);
    useEffect(() => {
    ...
    const messages = useMemo(() => Object.values(rawMessages).sort((a, b) => b.startBlockTime - a.startBlockTime), [rawMessages]);
        ...
        fetchMore: useFetcher
            ? async (num) => {
                await runFetch({
                    ...variables,
                    limit: num,
                    maxBlockTime: fetchedMessageParts &&
        ...
        fetchNew: useFetcher
            ? async (num) => {
                await runFetch({
                    ...
                    minBlockTime: fetchedMessageParts && fetchedMessageParts[0]
                        ? fetchedMessageParts[0].blockTime
                        : 0,
                });
            }
            : rest.fetchNew,
        loadingInitial: loadingInitial || (useFetcher && !fetchedMessageParts && !fetchLoading),
