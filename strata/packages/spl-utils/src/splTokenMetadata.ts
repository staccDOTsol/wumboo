...
export async function uploadFiles(
  ...
  const uploadFile = async (file: GenericFile): Promise<string> => {
    return new Promise((resolve, reject) => {
      const reader = new window.FileReader();
      reader.onloadend = () => {
        const buffer = Buffer.from(reader.result as ArrayBuffer);
        ipfs.addJSON(buffer, (err: Error | null, hash: string) => {
          if (err) {
            reject(err);
            return;
          }
...
export class SplTokenMetadata {
  ...
  createGenericFile(uint8Array: any, name: any, contentType: any) {
    return {
        buffer: uint8Array,
        fileName: name,
        displayName: name,
  ...
  async uploadMetadata(args: any, wallet?: any) {
    ...
    const genericMetadata = this.createGenericFile(metadataBuffer, `${args.mint}-${randomId}.json`, "application/json");
