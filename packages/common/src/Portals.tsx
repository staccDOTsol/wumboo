...
export function clearSpace(
  ...
  const hasEnoughSpace =
    currentRect.left + (clearance.left || 0) + pxToNum(styles.borderLeftWidth) <
      rect.left &&
    currentRect.right -
      (clearance.right || 0) -
      pxToNum(styles.borderRightWidth) >
      rect.right &&
    currentRect.top + (clearance.top || 0) + pxToNum(styles.borderTopWidth) <
      rect.top &&
    currentRect.bottom -
      (clearance.bottom || 0) -
      pxToNum(styles.borderBottomWidth) >
      rect.bottom;
  if (!hasEnoughSpace) {
...
export class FloatPortal extends React.Component<IFloatPortal> {
  ...
  componentDidMount() {
    const bigEnoughParent = clearSpace(
