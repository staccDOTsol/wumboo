...
export const TokenBondingRecentTransactions =
  createContext<ITokenBondingTransactions>({
    loading: false,
    hasMore: false,
  });
 ...
 * @returns
 */
async function getRecentBondingTransactions(
  connection: Connection,
  tokenBonding: ITokenBonding | undefined | null
): Promise<{ baseAmount: number; targetAmount: number }[]> {
  if (!tokenBonding) {
    return [];
  }
  ...
  return transactions
    ...
    .map((txn) => {
      ...
      if (txn.meta) {
        ...
        const preBaseBalance =
          txn.meta.preTokenBalances?.find(
            (t) => t.accountIndex === baseStorageIndex
          )?.uiTokenAmount.uiAmount || 0;
        const postBaseBalance =
...
export const TokenBondingRecentTransactionsProvider = ({
  children,
  tokenBonding,
}: {
