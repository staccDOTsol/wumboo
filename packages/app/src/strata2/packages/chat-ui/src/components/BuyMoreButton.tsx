...
const DefaultTrigger: BuyMoreTrigger = ({
  ...
  return (
    <Button
      ...
      {...btnProps}
    >
      {connected ? `Buy More ${metadata.symbol}` : "Connect Wallet"}
    </Button>
  );
};
  ...
  function onClick() {
    if (!connected) setVisible(true);
      ...
      if (!account && !loadingBonding) {
        window.open(`https://jup.ag/swap/SOL-${metadata.symbol.toUpperCase()}`);
      } else {
        onToggle();
      }
    }
    ...
    <>
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="2xl" isCentered trapFocus>
        ...
        <ModalContent borderRadius="xl" shadow="xl">
          <ModalHeader>Buy More {metadata.symbol}</ModalHeader>
