...
import {  useTokenBondingFromMint, useTokenMetadata } from "@strata-foundation/react";
import { useRouter } from "next/router";
import { routes, route } from "../utils/routes";
import { MetadataAccountData as MetadataData  } from "@metaplex-foundation/mpl-token-metadata";
...
export const TokenItem = ({mint, isIntermediateToken}: {
  ...
  const { image, data, data: metadata } = useTokenMetadata(mint);

  const hasAnyAuth = useMemo(() => {
    return publicKey && tokenBonding && (
      tokenBonding.reserveAuthority.equals(publicKey) ||
      tokenBonding.generalAuthority?.equals(publicKey)
    );
  }, [publicKey, tokenBonding]);
  // @ts-ignore
